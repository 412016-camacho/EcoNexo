...
public class GlobalHandlerException {
    ...
    public ResponseEntity<ErrorApi> handleError(ResponseStatusException ex) {
        ErrorApi error = buildError(ex.getMessage(), HttpStatus.valueOf(ex.getStatusCode().value()));
    ...
    public ResponseEntity<ErrorApi> handleError(ObjectOptimisticLockingFailureException ex) {
        ErrorApi error = buildError(ex.getMessage(), HttpStatus.CONFLICT);
        return ResponseEntity.status(HttpStatus.CONFLICT).body(error);
    }
